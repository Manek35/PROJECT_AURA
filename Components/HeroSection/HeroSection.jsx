...
const HeroSection = ({}) => {
  ...
  const callOutPut = async (value) => {
    ...
    console.log("output", networkConnect.chainId, tokenOne, tokenTwo);
    ...
    if (data) {
      setTokenSwapOutPut(data.estimatedOutput);
      const message = `${value} ${tokenOne.name} = ${data.estimatedOutput} ${tokenTwo.name}`;
      setPoolMessage(message);
    } else {
      setTokenSwapOutPut("0");
      setPoolMessage("Error fetching price");
    }
    setSearch(false);
      ...
      <div className={Style.HeroSection_box}>
        ...
        <div className={Style.HeroSection_box_input}>
          <input
            ...
            placeholder="0"
            onChange={(e) => (
              callOutPut(e.target.value), setSwapAmount(e.target.value)
            )}
          />
