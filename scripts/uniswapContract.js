        ...
        Object.keys(linkReferences[fileName]).forEach((contractName) => {
            ...
            linkReferences[fileName][contractName].forEach(({ start, length }) => {
                ...
                linkedBytecode = linkedBytecode
                    .slice(0, start2)
                    .concat(address)
                    .concat(linkedBytecode.slice(start2 + length2, linkedBytecode.length));
            });
        });
    });
...
async function main() {
    ...
    deploymentdata.nonfungibleTokenPositionDescriptor=nonfungibleTokenPositionDescriptor.address;
    deploymentdata.nonfungiblePositionManager=nonfungiblePositionManager.address;
    fs.writeFileSync(
